...
static gboolean gst_nvinfer_parse_class_attrs(
  ...
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_THRESHOLD)) {
      ...
      detection_params.preClusterThreshold = g_key_file_get_double(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_PRE_CLUSTER_THRESHOLD,
          &error);
      CHECK_ERROR(error);
      if (detection_params.preClusterThreshold < 0) {
        g_printerr(
            "Error: Negative pre cluster threshold (%.5f) specified for group "
            "%s\n",
            detection_params.preClusterThreshold, group);
        goto done;
      }
      ...
      if (detection_params.postClusterThreshold < 0) {
        g_printerr(
            "Error: Negative post cluster threshold (%.5f) specified for group "
            "%s\n",
      ...
      detection_params.groupThreshold = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_GROUP_THRESHOLD,
          &error);
      CHECK_ERROR(error);
      ...
      if (detection_params.minBoxes < 0) {
        g_printerr("Error: Negative minBoxes (%d) specified for group %s\n",
      ...
      detection_params.minScore = g_key_file_get_double(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DBSCAN_MIN_SCORE,
          &error);
      CHECK_ERROR(error);
      if (detection_params.minScore < 0) {
        g_printerr("Error: Negative minScore (%f) specified for group %s\n",
      ...
      detection_filter_params.roiTopOffset = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_TOP_OFFSET,
          &error);
      CHECK_ERROR(error);
      if ((gint)detection_filter_params.roiTopOffset < 0) {
        g_printerr("Error: Negative roiTopOffset (%d) specified for group %s\n",
      ...
      detection_filter_params.roiBottomOffset = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_ROI_BOTTOM_OFFSET,
          &error);
      CHECK_ERROR(error);
      ...
      detection_filter_params.detectionMinWidth = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_WIDTH,
          &error);
      CHECK_ERROR(error);
      ...
      detection_filter_params.detectionMinHeight = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MIN_HEIGHT,
          &error);
      CHECK_ERROR(error);
      ...
      detection_filter_params.detectionMaxWidth = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_WIDTH,
          &error);
      CHECK_ERROR(error);
      ...
      detection_filter_params.detectionMaxHeight = g_key_file_get_integer(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_DETECTED_MAX_HEIGHT,
          &error);
      ...
      gdouble *list = g_key_file_get_double_list(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_BORDER_COLOR, &length,
          &error);
      CHECK_ERROR(error);
      if (length != 4) {
        g_printerr(
      ...
      gsize length;
      gdouble *list = g_key_file_get_double_list(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_BG_COLOR, &length,
          &error);
      CHECK_ERROR(error);
      if (length != 4) {
        g_printerr(
      ...
      detection_params.nmsIOUThreshold = g_key_file_get_double(
          key_file, group, CONFIG_GROUP_INFER_CLASS_ATTRS_NMS_IOU_THRESHOLD,
          &error);
      CHECK_ERROR(error);
...
static gboolean gst_nvinfer_parse_other_attribute(GstNvInfer *nvinfer,
                                                  GKeyFile *key_file,
  ...
  if (!g_strcmp0(key, CONFIG_GROUP_INFER_PROCESS_MODE)) {
    if ((*nvinfer->is_prop_set)[PROP_PROCESS_MODE]) return TRUE;
    ...
    if ((*nvinfer->is_prop_set)[PROP_INTERVAL]) return TRUE;
    ...
    nvinfer->secondary_reinfer_interval = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_SECONDARY_REINFER_INTERVAL,
        &error);
    CHECK_ERROR(error);
    ...
    nvinfer->min_input_object_width = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_WIDTH,
        &error);
    CHECK_ERROR(error);
    ...
    nvinfer->min_input_object_height = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MIN_HEIGHT,
        &error);
    CHECK_ERROR(error);
    ...
    nvinfer->max_input_object_width = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_WIDTH,
        &error);
    CHECK_ERROR(error);
    ...
    nvinfer->max_input_object_height = g_key_file_get_integer(
        key_file, group_name, CONFIG_GROUP_INFER_INPUT_OBJECT_MAX_HEIGHT,
        &error);
    CHECK_ERROR(error);
    ...
    gint *int_list = g_key_file_get_integer_list(
        key_file, group_name, CONFIG_GROUP_INFER_CLASS_IDS_FOR_OPERATION,
        &length, &error);
    CHECK_ERROR(error);

    for (i = 0; i < length; i++) {
      if (int_list[i] > max_class_id) max_class_id = int_list[i];
    }
    ...
    gint *int_list = g_key_file_get_integer_list(
        key_file, group_name, CONFIG_GROUP_INFER_CLASS_IDS_FOR_FILTERING,
        &length, &error);
    CHECK_ERROR(error);
...
static gboolean gst_nvinfer_parse_props(GstNvInfer *nvinfer,
  ...
  if (nvinfer) nvinfer->secondary_reinfer_interval = DEFAULT_REINFER_INTERVAL;

  init_params->networkInputFormat = NvDsInferFormat_RGB;
  for (key = keys; *key; key++) {
    if (!g_strcmp0(*key, CONFIG_GROUP_INFER_UNIQUE_ID)) {
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_UNIQUE_ID]) continue;
      ...
      if (nvinfer) nvinfer->unique_id = init_params->uniqueID;
      ...
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_GPU_DEVICE_ID]) continue;
      gint devices;
      ...
      if (nvinfer) nvinfer->gpu_id = init_params->gpuID;
      ...
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_BATCH_SIZE]) continue;
      ...
      if (nvinfer) nvinfer->max_batch_size = init_params->maxBatchSize;
      ...
      gsize length;
      gint *int_list = g_key_file_get_integer_list(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_INFER_DIMENSIONS,
          &length, &error);
      CHECK_ERROR(error);
      ...
      init_params->inferInputDims =
          NvDsInferDimsCHW{(unsigned int)int_list[0], (unsigned int)int_list[1],
      ...
      if (nvinfer && (*nvinfer->is_prop_set)[PROP_MODEL_ENGINEFILE]) continue;
      gchar *str =
      ...
      init_params->outputLayerNames = g_key_file_get_string_list(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_OUTPUT_BLOB_NAMES,
          &length, &error);
      ...
      init_params->outputIOFormats = g_key_file_get_string_list(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_OUTPUT_IO_FORMATS,
          &length, &error);
      ...
      gdouble *dbl_list = g_key_file_get_double_list(
          key_file, CONFIG_GROUP_PROPERTY, CONFIG_GROUP_INFER_OFFSETS, &length,
          &error);
      CHECK_ERROR(error);
      ...
      for (i = 0; i < length; i++) init_params->offsets[i] = dbl_list[i];
      ...
      if (!g_key_file_has_key(key_file, CONFIG_GROUP_PROPERTY,
        ...
        g_printerr(
            "Warning: 'input-dims' parameter has been deprecated. Use "
            "'infer-dims' instead.\n");
        gsize length;
        gint *int_list = g_key_file_get_integer_list(
            key_file, CONFIG_GROUP_PROPERTY,
            CONFIG_GROUP_INFER_UFF_INPUT_DIMENSIONS_LEGACY, &length, &error);
        CHECK_ERROR(error);
        ...
        g_printerr(
            "Warning: Ignoring 'input-dims' parameter since 'infer-dims' has "
            "been set.\n");
    } else if (!g_strcmp0(*key,
      ...
      switch (val) {
        ...
        default:
          g_printerr(
              "Error. Invalid value for '%s', Segmentation output order "
              ":'%d'\n",
  ...
  if (init_params->networkType == NvDsInferNetworkType_Detector ||
    ...
    if (g_key_file_has_group(cfg_file,
      ...
      if (!gst_nvinfer_parse_class_attrs(
              cfg_file, (gchar *)CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "all",
    ...
    for (group = groups; *group; group++) {
      if (!strncmp(*group, CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX,
        ...
        if (class_index == 0 && endptr == key1) {
          g_printerr(
              "Invalid group [%s]. Class attributes should be specified using "
              "group name '" CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX
              "<class-id>'\n",
              *group);
          goto done;
        }
