...
void LockGMutex::lock() {
  assert(!locked);
  if (!locked) g_mutex_lock(&m);
  locked = true;
}

void LockGMutex::unlock() {
  assert(locked);
  if (locked) g_mutex_unlock(&m);
  locked = false;
}

void LockGMutex::wait(GCond &cond) {
  assert(locked);
  if (locked) g_cond_wait(&cond, &m);
}
...
void DsNvInferImpl::loadModel(const std::string &modelPath,
                              ModelLoadType loadType) {
  ...
  if (!needUpdate) {
    notifyLoadModelStatus(ModelStatus{NVDSINFER_UNKNOWN_ERROR, modelPath,
  ...
  if (!initNewInferModelParams(newParams, modelPath, loadType, curParam)) {
    notifyLoadModelStatus(
        ModelStatus{NVDSINFER_CONFIG_FAILED, modelPath,
  ...
  NvDsInferStatus err = createNvDsInferContext(&newContext, newParams,
                                               m_GstInfer, gst_nvinfer_logger);
...
bool DsNvInferImpl::initNewInferModelParams(
    NvDsInferContextInitParams &newParams, const std::string &newModelPath,
    ModelLoadType loadType, const NvDsInferContextInitParams &oldParams) {
  ...
  if (oldParams.numDetectedClasses) {
    ...
    memcpy(newParams.perClassDetectionParams, oldParams.perClassDetectionParams,
...
bool DsNvInferImpl::isNewContextValid(INvDsInferContext &newCtx,
  ...
  if (newParam.maxBatchSize < m_GstInfer->max_batch_size) {
    GST_WARNING_OBJECT(m_GstInfer,
                       "[UID %d]: New model batch-size[in config] (%d) is "
                       "smaller then gst-nvinfer's"
                       " configured batch-size (%d).",
                       m_GstInfer->unique_id, newParam.maxBatchSize,
  ...
  if (m_GstInfer->network_width != (gint)networkInfo.width ||
    ...
    GST_WARNING_OBJECT(
        ...
        m_GstInfer->unique_id, networkInfo.width, networkInfo.height,
        m_GstInfer->network_width, m_GstInfer->network_height);
    return false;
  }
...
NvDsInferStatus DsNvInferImpl::ensureReplaceNextContext() {
  ...
  if (!nextReplacement.get()) return NVDSINFER_SUCCESS;
