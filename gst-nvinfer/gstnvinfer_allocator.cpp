...
#define GST_NVINFER_ALLOCATOR(obj)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), GST_TYPE_NVINFER_ALLOCATOR, \
                              GstNvInferAllocator))
#define GST_NVINFER_ALLOCATOR_CLASS(klass)                      \
  (G_TYPE_CHECK_CLASS_CAST((klass), GST_TYPE_NVINFER_ALLOCATOR, \
 ...
 */
#define _do_init                                                              \
  GST_DEBUG_CATEGORY_INIT(gst_nvinfer_allocator_debug, "nvinferallocator", 0, \
                          "nvinfer allocator");
...
static void gst_nvinfer_allocator_free(GstAllocator *allocator,
                                       GstMemory *memory) {
...
static gpointer gst_nvinfer_memory_map(GstMemory *mem, gsize maxsize,
                                       GstMapFlags flags) {
...
GstNvInferMemory *gst_nvinfer_buffer_get_memory(GstBuffer *buffer) {
  ...
  if (!mem || !gst_memory_is_type(mem, GST_NVINFER_MEMORY_TYPE)) return nullptr;
