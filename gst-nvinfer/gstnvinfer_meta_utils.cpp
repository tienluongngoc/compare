...
void attach_metadata_detector(GstNvInfer *nvinfer,
  ...
  for (guint i = 0; i < detection_output.numObjects; i++) {
    ...
    if (nvinfer->filter_out_class_ids->find(obj.classIndex) !=
      ...
      continue;
    if (obj.width < filter_params.detectionMinWidth) continue;
    if (obj.height < filter_params.detectionMinHeight) continue;
    ...
    if (filter_params.detectionMaxHeight > 0 &&
      ...
      continue;
    if (obj.top < filter_params.roiTopOffset) continue;
    ...
    if (obj.label) strncpy(obj_meta->obj_label, obj.label, MAX_LABEL_SIZE);
...
static gpointer copy_segmentation_meta(gpointer data, gpointer user_data) {
  ...
  meta->class_probabilities_map = (gfloat *)g_memdup(
...
void attach_metadata_segmentation(
    GstNvInfer *nvinfer, GstMiniObject *tensor_out_object,
