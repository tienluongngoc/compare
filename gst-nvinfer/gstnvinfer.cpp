...
#define DEFAULT_ATTACH_POINTS FALSE      // 05-09-2021 kienvt
#define DEFAULT_TRANSFORM_OBJECTS FALSE  // 06-09-2021 kienvt
...
static GstFlowReturn gst_nvinfer_submit_input_buffer(GstBaseTransform *btrans,
                                                     gboolean discont,
                                                     GstBuffer *inbuf);
static GstFlowReturn gst_nvinfer_generate_output(GstBaseTransform *btrans,
                                                 GstBuffer **outbuf);
...
static void gst_nvinfer_class_init(GstNvInferClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum(
          "process-mode", "Process Mode", "Infer processing mode",
          GST_TYPE_NVDSINFER_PROCESS_MODE, DEFAULT_PROCESS_MODE,
          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
                        GST_PARAM_MUTABLE_READY)));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint(
          "batch-size", "Batch Size", "Maximum batch size for inference", 1,
          NVDSINFER_MAX_BATCH_SIZE, DEFAULT_BATCH_SIZE,
          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
                        GST_PARAM_MUTABLE_READY)));
  ...
  g_object_class_install_property(
      gobject_class, PROP_OPERATE_ON_GIE_ID,
      g_param_spec_int(
          "operate-on-gie-id", "Infer on Gie ID",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "operate-on-class-ids", "Operate on Class ids",
  ...
  g_object_class_install_property(
      gobject_class, PROP_FILTER_OUT_CLASS_IDS,
      g_param_spec_string(
          "filter-out-class-ids", "Ignore metadata for class ids",
  ...
  g_object_class_install_property(
      gobject_class, PROP_MODEL_ENGINEFILE,
      g_param_spec_string(
  ...
  g_object_class_install_property(
      ...
      g_param_spec_uint(
          "gpu-id", "Set GPU Device ID", "Set GPU Device ID", 0, G_MAXUINT,
          DEFAULT_GPU_DEVICE_ID,
          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
                        GST_PARAM_MUTABLE_READY)));

  g_object_class_install_property(
      gobject_class, PROP_OUTPUT_WRITE_TO_FILE,
      g_param_spec_boolean(
          "raw-output-file-write", "Raw Output File Write",
          "Write raw inference output to file", DEFAULT_OUTPUT_WRITE_TO_FILE,
          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
                        GST_PARAM_MUTABLE_READY)));
  ...
  g_object_class_install_property(
      gobject_class, PROP_OUTPUT_TENSOR_META,
      g_param_spec_boolean(
          "output-tensor-meta", "Output Tensor Meta",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean(
          "output-instance-mask", "Output Instance Mask",
  ...
  // 06-09-2021 kienvt
  g_object_class_install_property(
      gobject_class, PROP_TRANSFORM_OBJECTS,
      g_param_spec_boolean(
          "transform-objects", "Transform Objects",
          "Transform objects by using warpPerspective() before inferencing.",
          DEFAULT_TRANSFORM_OBJECTS,
          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
                        GST_PARAM_MUTABLE_READY)));
  ...
  gst_nvinfer_signals[SIGNAL_MODEL_UPDATED] =
      g_signal_new("model-updated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GstNvInferClass, model_updated), NULL, NULL,
  ...
  gst_element_class_set_details_simple(
      gstelement_class, "NvInfer plugin", "NvInfer Plugin",
...
static void gst_nvinfer_init(GstNvInfer *nvinfer) {
  ...
  nvinfer->impl =
      reinterpret_cast<GstNvInferImpl *>(new DsNvInferImpl(nvinfer));
  ...
  nvinfer->attach_points = DEFAULT_ATTACH_POINTS;          // 05-09-2021 kienvt
  nvinfer->transform_objects = DEFAULT_TRANSFORM_OBJECTS;  // 06-09-2021 kienvt
...
static void gst_nvinfer_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_OPERATE_ON_CLASS_IDS: {
      ...
      for (auto &cid : class_ids) nvinfer->operate_on_class_ids->at(cid) = TRUE;
    } break;
    ...
    case PROP_OUTPUT_INSTANCE_MASK:
      ...
      break;
    case PROP_TRANSFORM_OBJECTS:
      nvinfer->transform_objects = g_value_get_boolean(value);
      break;
    default:
...
static void gst_nvinfer_get_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_UNIQUE_ID:
      ...
      break;
    case PROP_PROCESS_MODE:
      g_value_set_enum(value, nvinfer->process_full_frame
    ...
    case PROP_OPERATE_ON_CLASS_IDS: {
      ...
      for (size_t i = 0; i < nvinfer->operate_on_class_ids->size(); i++) {
        if (nvinfer->operate_on_class_ids->at(i)) str << i << ":";
      }
      g_value_set_string(value, str.str().c_str());
    } break;
    case PROP_FILTER_OUT_CLASS_IDS: {
      std::stringstream str;
      for (const auto id : *nvinfer->filter_out_class_ids) str << id << ";";
      g_value_set_string(value, str.str().c_str());
    } break;
    case PROP_MODEL_ENGINEFILE:
    ...
    case PROP_OUTPUT_INSTANCE_MASK:
      ...
      break;
    case PROP_TRANSFORM_OBJECTS:
      g_value_set_boolean(value, nvinfer->transform_objects);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
      break;
...
void gst_nvinfer_logger(NvDsInferContextHandle handle, unsigned int unique_id,
                        NvDsInferLogLevel log_level, const char *log_message,
                        void *user_ctx) {
  GstNvInfer *nvinfer = GST_NVINFER(user_ctx);

  switch (log_level) {
    case NVDSINFER_LOG_ERROR:
      GST_ERROR_OBJECT(nvinfer, "NvDsInferContext[UID %d]: %s", unique_id,
                       log_message);
      return;
    case NVDSINFER_LOG_WARNING:
      GST_WARNING_OBJECT(nvinfer, "NvDsInferContext[UID %d]: %s", unique_id,
                         log_message);
      return;
    case NVDSINFER_LOG_INFO:
      GST_INFO_OBJECT(nvinfer, "NvDsInferContext[UID %d]: %s", unique_id,
                      log_message);
      return;
    case NVDSINFER_LOG_DEBUG:
      GST_DEBUG_OBJECT(nvinfer, "NvDsInferContext[UID %d]: %s", unique_id,
                       log_message);
      return;
  }
}
...
static gboolean gst_nvinfer_start(GstBaseTransform *btrans) {
  ...
  status = createNvDsInferContext(&infer_context, *init_params, nvinfer,
                                  gst_nvinfer_logger);
  if (status != NVDSINFER_SUCCESS) {
    GST_ELEMENT_ERROR(nvinfer, RESOURCE, FAILED,
                      ...
                      ("Config file path: %s, NvDsInfer Error: %s",
                       nvinfer->config_file_path, NvDsInferStatus2Str(status)));
    return FALSE;
  }
  ...
  for (auto &layer : *(nvinfer->layers_info)) {
    if (!layer.isInput) nvinfer->output_layers_info->push_back(layer);
  }
  ...
  gst_buffer_pool_config_set_params(
      config_ptr.get(), nullptr, sizeof(GstNvInferMemory),
  ...
  std::unique_ptr<GstAllocator, decltype(allocator_deleter)> allocator_ptr(
      gst_nvinfer_allocator_new(nvinfer->network_width, nvinfer->network_height,
                                color_format, nvinfer->max_batch_size,
                                nvinfer->gpu_id),
      allocator_deleter);
  ...
  // 07-09-2021 kienvt
  if (nvinfer->transform_objects) {
    nvinfer->cv_cuda_stream_ptr =
        std::make_unique<cv::cuda::Stream>(cudaStreamNonBlocking);
  }
  ...
  impl->notifyLoadModelStatus(ModelStatus{NVDSINFER_SUCCESS,
...
static gboolean gst_nvinfer_stop(GstBaseTransform *btrans) {
  ...
  if (nvinfer->convertStream) cudaStreamDestroy(nvinfer->convertStream);
...
static GstFlowReturn get_converted_buffer(
    GstNvInfer *nvinfer, NvBufSurface *src_surf, NvBufSurfaceParams *src_frame,
    NvOSD_RectParams *crop_rect_params, NvBufSurface *dest_surf,
    NvBufSurfaceParams *dest_frame, gdouble &ratio_x, gdouble &ratio_y,
    void *destCudaPtr) {
...
// 06-09-2021 kienvt
#if 0
  if (nvinfer->transform_objects && nvinfer->current_batch_num == 11) {
    static int count = -1;
    count++;
    NvBufSurface *surface = mem->surf;
    void *data_ptr = surface->surfaceList[0].dataPtr;
    int size = surface->surfaceList[0].dataSize;
    int height = surface->surfaceList[0].height;
    int width = surface->surfaceList[0].width;
    int pitch = surface->surfaceList[0].pitch;
    auto color_format = surface->surfaceList[0].colorFormat;
    auto src_plane_params = surface->surfaceList[0].planeParams;

    if (color_format == NVBUF_COLOR_FORMAT_GRAY8) {
      cv::cuda::GpuMat gpu_mat = cv::cuda::GpuMat(height, width, CV_8UC1, data_ptr, pitch);
      cv::Mat cpu_mat;
      gpu_mat.download(cpu_mat, *nvinfer->cv_cuda_stream_ptr);
      cv::imwrite(fmt::format("test_transformed_gray_{}.jpg", count), cpu_mat);
    } else if (color_format == NVBUF_COLOR_FORMAT_RGB) {
      cv::cuda::GpuMat gpu_mat = cv::cuda::GpuMat(height, width, CV_8UC3, data_ptr, pitch);
      cv::Mat cpu_mat;
      gpu_mat.download(cpu_mat, *nvinfer->cv_cuda_stream_ptr);
      cv::cvtColor(cpu_mat, cpu_mat, cv::COLOR_RGB2BGR);
      cv::imwrite(fmt::format("test_transformed_color_{}.jpg", count), cpu_mat);
    }
  }
#endif
...
static GstFlowReturn gst_nvinfer_process_full_frame(GstNvInfer *nvinfer,
                                                    GstBuffer *inbuf,
  ...
  if (((in_surf->memType == NVBUF_MEM_DEFAULT ||
    ...
    GST_ELEMENT_ERROR(
        ...
        ("surface-gpu-id=%d,%s-gpu-id=%d", in_surf->gpuId,
         GST_ELEMENT_NAME(nvinfer), nvinfer->gpu_id));
    return GST_FLOW_ERROR;
  }
  ...
  for (guint i = 0; i < num_filled; i++) {
    ...
    if (get_converted_buffer(
            nvinfer, in_surf, in_surf->surfaceList + i, &rect_params,
    ...
    if (batch->frames.size() == nvinfer->max_batch_size ||
        i == num_filled - 1) {
      if (!convert_batch_and_push_to_input_thread(nvinfer, batch.get(),
                                                  memory)) {
        return GST_FLOW_ERROR;
      }
...
static inline gboolean should_infer_object(GstNvInfer *nvinfer,
                                           GstBuffer *inbuf,
...
static GstFlowReturn gst_nvinfer_process_objects(GstNvInfer *nvinfer,
                                                 GstBuffer *inbuf,
                                                 NvBufSurface *in_surf) {
  std::unique_ptr<GstNvInferBatch> batch(nullptr);
  GstBuffer *conv_gst_buf = nullptr;
  ...
  std::vector<cv::cuda::GpuMat> backup_source_mats;
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL;
       l_frame = l_frame->next) {
    NvDsFrameMeta *frame_meta = (NvDsFrameMeta *)(l_frame->data);
    NvBufSurfaceParams src_frame = in_surf->surfaceList[frame_meta->batch_id];
    void *src_data_ptr = src_frame.dataPtr;
    auto src_width = src_frame.width;
    auto src_height = src_frame.height;
    auto src_pitch = src_frame.pitch;

    // 01/12/2021 tienln fix bug bbox out of frame
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL;
         l_obj = l_obj->next) {
      obj_meta = (NvDsObjectMeta *)(l_obj->data);
      obj_meta->rect_params.left = (obj_meta->rect_params.left > 0.0f)
                                       ? obj_meta->rect_params.left
                                       : 0.0f;
      obj_meta->rect_params.top =
          (obj_meta->rect_params.top > 0.0f) ? obj_meta->rect_params.top : 0.0f;
      obj_meta->rect_params.width = (obj_meta->rect_params.width < src_width)
                                        ? obj_meta->rect_params.width
                                        : src_width;
      obj_meta->rect_params.width = (obj_meta->rect_params.width < src_height)
                                        ? obj_meta->rect_params.width
                                        : src_height;
    }
    cv::cuda::GpuMat src_mat(src_height * 1.5, src_width, CV_8UC1, src_data_ptr,
                             src_pitch);
    cv::cuda::GpuMat backup_src_mat(src_height * 1.5, src_width, CV_8UC1,
                                    cv::Scalar(255));
    cv::cuda::GpuMat backup_src_mat_roi =
        backup_src_mat(cv::Range(0, src_height * 1.5), cv::Range(0, src_width));
    src_mat.copyTo(backup_src_mat_roi);
    backup_source_mats.push_back(backup_src_mat);
  }
  ...
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      bool needs_infer = should_infer_object(nvinfer, inbuf, object_meta,
                                             frame_num, obj_history.get());
      if (!needs_infer) {
        ...
        if (IS_CLASSIFIER_INSTANCE(nvinfer) && obj_history != nullptr) {
          ...
          if (!nvinfer->classifier_async_mode) {
            ...
            if (batch == nullptr) {
              ...
              batch->inbuf_batch_num = nvinfer->current_batch_num;
              locker.unlock();
              flow_ret = gst_buffer_pool_acquire_buffer(nvinfer->pool,
                                                        &conv_gst_buf, nullptr);
              locker.lock();
              if (flow_ret != GST_FLOW_OK) {
                return flow_ret;
            ...
            batch->objs_pending_meta_attach.emplace_back(obj_history,
                                                         object_meta);
            continue;
          }
        }
      ...
      if (batch == nullptr) {
        ...
        flow_ret = gst_buffer_pool_acquire_buffer(nvinfer->pool, &conv_gst_buf,
                                                  nullptr);
        if (flow_ret != GST_FLOW_OK) {
          return flow_ret;
        }
      ...
      // 06-09-2021 kienvt
      GstFlowReturn ret;
      bool should_align = nvinfer->transform_objects &&
                          object_meta->mask_params.size > 0 &&
                          object_meta->mask_params.width == 3 &&
                          object_meta->mask_params.height == 3;

      if (should_align) {
        float *t_data = object_meta->mask_params.data;
        int t_width = object_meta->mask_params.width;
        int t_height = object_meta->mask_params.height;
        cv::Mat T(t_width, t_height, CV_32F, t_data);
        uint8_t *imgPtr;
        // if (clone_data_ptr == nullptr) {
        //   cudaMallocPitch(&clone_data_ptr, &clone_pitch, src_width,
        //   src_height * 1.5); cudaMemcpy2DAsync(clone_data_ptr, clone_pitch,
        //   src_data_ptr, src_pitch, src_width, src_height * 1.5,
        //   cudaMemcpyDeviceToDevice, nvinfer->convertStream);

        //   src_frame.dataPtr = clone_data_ptr;
        //   src_frame.pitch = clone_pitch;

        //   src_frame.planeParams.pitch[0] = clone_pitch;
        //   src_frame.planeParams.pitch[1] = clone_pitch;
        //   src_frame.planeParams.psize[0] = clone_pitch * src_height;
        //   src_frame.planeParams.psize[1] = clone_pitch * src_height / 2;
        //   src_frame.planeParams.offset[1] = clone_pitch * src_height;
        // }
        // switch (src_frame.colorFormat) {
        //   case NVBUF_COLOR_FORMAT_NV12_709:
        //   case NVBUF_COLOR_FORMAT_NV12:
        //     break;
        //   default:
        //     g_printerr("color_format_code = %d is not supported.",
        //     src_frame.colorFormat); return GST_FLOW_ERROR;
        // }
        cv::cuda::GpuMat src_mat(src_height * 1.5, src_width, CV_8UC1,
                                 src_data_ptr, src_pitch);
        // cv::cuda::GpuMat clone_mat(src_height * 1.5, src_width, CV_8UC1,
        // clone_data_ptr, clone_pitch);

        NvOSD_RectParams crop_rect_params = object_meta->rect_params;
        int left = static_cast<int>(crop_rect_params.left);
        int top = static_cast<int>(crop_rect_params.top);
        int width = static_cast<int>(crop_rect_params.width);
        int height = static_cast<int>(crop_rect_params.height);

        cv::cuda::GpuMat dst_mat;
        cv::cuda::warpPerspective(src_mat, dst_mat, T, cv::Size(width, height),
                                  cv::INTER_LINEAR, cv::BORDER_CONSTANT,
                                  cv::Scalar(), *nvinfer->cv_cuda_stream_ptr);
        cv::cuda::GpuMat roi_mat = src_mat(cv::Range(top, top + height),
                                           cv::Range(left, left + width));
        dst_mat.copyTo(roi_mat, *nvinfer->cv_cuda_stream_ptr);

        ret = get_converted_buffer(
            nvinfer, in_surf, in_surf->surfaceList + frame_meta->batch_id,
            &crop_rect_params, memory->surf, memory->surf->surfaceList + idx,
            scale_ratio_x, scale_ratio_y, memory->frame_memory_ptrs[idx]);
      } else {
        ret = get_converted_buffer(
            nvinfer, in_surf, in_surf->surfaceList + frame_meta->batch_id,
    ...
    if (clone_data_ptr) {
      cudaFree(clone_data_ptr);  // 07-09-2021 kienvt
    }
  }
  ...
  if (batch) {
    ...
    if (batch->frames.size() == 0) gst_buffer_unref(batch->conv_buf);
  ...
  for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    cv::cuda::GpuMat src_mat(src_height * 1.5, src_width, CV_8UC1, src_data_ptr,
                             src_pitch);
    cv::cuda::GpuMat bk_src_roi =
        src_mat(cv::Range(0, src_height * 1.5), cv::Range(0, src_width));
    backup_source_mats[frame_meta->batch_id].copyTo(bk_src_roi);
  }
...
static GstFlowReturn gst_nvinfer_submit_input_buffer(GstBaseTransform *btrans,
  ...
  if (impl->ensureReplaceNextContext() != NVDSINFER_SUCCESS) {
    GST_ELEMENT_ERROR(nvinfer, RESOURCE, FAILED,
                      ("Ensure next context failed."), ("streaming stopped"));
    return GST_FLOW_ERROR;
  }
  ...
  if (in_map_info.data) gst_buffer_unmap(inbuf, &in_map_info);

  if (flow_ret == GST_FLOW_ERROR) return GST_FLOW_ERROR;
...
static void gst_nvinfer_output_generated_file_write(
    GstBuffer *buf, NvDsInferNetworkInfo *network_info,
...
static gpointer gst_nvinfer_output_loop(gpointer data) {
  ...
  while (!nvinfer->stop) {
    ...
    auto tensor_deleter = [](GstNvInferTensorOutputObject *o) {
      if (o) gst_mini_object_unref(GST_MINI_OBJECT(o));
    };
    ...
    gst_mini_object_init(GST_MINI_OBJECT(tensor_out_object.get()), 0,
    ...
    if (status != NVDSINFER_SUCCESS) {
      GST_ELEMENT_ERROR(nvinfer, STREAM, FAILED,
                        ("Failed to dequeue output from inferencing. "
                         "NvDsInferContext error: %s",
    ...
    if (nvinfer->write_raw_buffers_to_file) {
      gst_nvinfer_output_generated_file_write(
          batch->inbuf, &nvinfer->network_info, nvinfer->layers_info->data(),
    ...
    if (nvinfer->output_generated_callback) {
      nvinfer->output_generated_callback(
          batch->inbuf, &nvinfer->network_info, nvinfer->layers_info->data(),
          nvinfer->layers_info->size(), batch->frames.size(),
    ...
    for (guint i = 0; i < batch->frames.size(); i++) {
      ...
      if (IS_DETECTOR_INSTANCE(nvinfer) ||
        ...
        attach_metadata_detector(
            nvinfer, GST_MINI_OBJECT(tensor_out_object.get()), frame,
            frame_output.detectionOutput, init_params->segmentationThreshold);
        ...
        attach_metadata_segmentation(nvinfer,
    ...
    if (nvinfer->output_tensor_meta && !nvinfer->classifier_async_mode) {
      ...
      attach_tensor_output_meta(nvinfer,
                                GST_MINI_OBJECT(tensor_out_object.get()),
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, nvdsgst_infer,
                  DESCRIPTION, nvinfer_plugin_init, "5.1", LICENSE,
                  BINARY_PACKAGE, URL)
