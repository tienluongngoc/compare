...
class LockGMutex {
 public:
  LockGMutex(GMutex &mutex) : m(mutex) { lock(); }
  ~LockGMutex() {
    if (locked) unlock();
  }
  void lock();
...
class DsNvInferImpl {
 public:
  using ContextReplacementPtr =
      std::unique_ptr<std::tuple<NvDsInferContextPtr,
                                 NvDsInferContextInitParamsPtr, std::string>>;
 ...
 private:
  ...
  bool initNewInferModelParams(NvDsInferContextInitParams &newParams,
                               const std::string &newModelPath,
                               ModelLoadType loadType,
                               const NvDsInferContextInitParams &oldParams);
  bool isNewContextValid(INvDsInferContext &newCtx,
                         NvDsInferContextInitParams &newParam);
  bool triggerContextReplace(NvDsInferContextPtr ctx,
  ...
  NvDsInferStatus resetContextUnlock(NvDsInferContextPtr ctx,
                                     NvDsInferContextInitParamsPtr params,
                                     const std::string &path);
