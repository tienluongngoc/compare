...
/**
 * Implementation of ModelParser for custom models. This implementation will
 * look for the function symbol "NvDsInferCreateModelParser" in the custom
 * library handle passed to it. It will call the NvDsInferCreateModelParser to
 * get an instance of the IModelParser implementation required to parse the
 * user's custom model.
 */
...
class TrtModelBuilder {
 public:
   ...
   * instance.
   */
  std::unique_ptr<TrtEngine> buildEngine(nvinfer1::INetworkDefinition& network,
                                         BuildParams& options);

  /* Serialize engine to file
   */
  NvDsInferStatus serializeEngine(const std::string& path,
                                  nvinfer1::ICudaEngine& engine);

  /* Deserialize engine from file
   */
  std::unique_ptr<TrtEngine> deserializeEngine(const std::string& path,
                                               int dla = -1);
 ...
 private:
  ...
  std::unique_ptr<BuildParams> createDynamicParams(
      const NvDsInferContextInitParams& initParams);
  void initCommonParams(BuildParams& params,
                        const NvDsInferContextInitParams& initParams);
