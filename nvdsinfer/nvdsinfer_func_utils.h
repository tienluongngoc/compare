...
#define dsInferError(fmt, ...)                                     \
  do {                                                             \
    dsInferLogPrint__(NVDSINFER_LOG_ERROR, INFER_LOG_FORMAT_(fmt), \
                      ##__VA_ARGS__);                              \
  } while (0)

#define dsInferWarning(fmt, ...)                                     \
  do {                                                               \
    dsInferLogPrint__(NVDSINFER_LOG_WARNING, INFER_LOG_FORMAT_(fmt), \
                      ##__VA_ARGS__);                                \
  } while (0)

#define dsInferInfo(fmt, ...)                                     \
  do {                                                            \
    dsInferLogPrint__(NVDSINFER_LOG_INFO, INFER_LOG_FORMAT_(fmt), \
                      ##__VA_ARGS__);                             \
  } while (0)

#define dsInferDebug(fmt, ...)                                     \
  do {                                                             \
    dsInferLogPrint__(NVDSINFER_LOG_DEBUG, INFER_LOG_FORMAT_(fmt), \
                      ##__VA_ARGS__);                              \
  } while (0)
...
#define RETURN_CUDA_ERR(err, fmt, ...) \
  CHECK_CUDA_ERR_W_ACTION(err, return NVDSINFER_CUDA_ERROR, fmt, ##__VA_ARGS__)
...
inline const char* safeStr(const char* str) { return !str ? "" : str; }

inline const char* safeStr(const std::string& str) { return str.c_str(); }

inline bool string_empty(const char* str) { return !str || strlen(str) == 0; }
...
inline void convertFullDims(const nvinfer1::Dims& fullDims,
