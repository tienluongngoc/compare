...
int TrtBackendContext::getLayerIdx(const std::string& bindingName) {
  assert(m_CudaEngine);
...
NvDsInferStatus ImplicitTrtBackendContext::initialize() {
...
NvDsInferStatus ImplicitTrtBackendContext::enqueueBuffer(
...
NvDsInferStatus DlaImplicitTrtBackendContext::enqueueBuffer(
    const std::shared_ptr<InferBatchBuffer>& buffer, CudaStream& stream,
    CudaEvent* consumeEvent) {
  assert(m_Context);
  assert(m_MaxBatchSize > 0);
...
NvDsInferStatus FullDimTrtBackendContext::enqueueBuffer(
  ...
  for (int iL = 0; iL < (int)m_AllLayers.size(); ++iL) {
    if (!m_AllLayers[iL].isInput) continue;
