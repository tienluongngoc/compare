...
void SplitFullDims(const nvinfer1::Dims& fullDims, NvDsInferDims& dims,
                   int& batch) {
  if (!fullDims.nbDims) {
    dims.numDims = 0;
    dims.numElements = 0;
...
std::string dims2Str(const NvDsInferDims& d) { return dims2Str(ds2TrtDims(d)); }
...
bool hasWildcard(const nvinfer1::Dims& dims) {
  return std::any_of(dims.d, dims.d + dims.nbDims,
                     [](int d) { return d == -1; });
}
...
bool operator==(const NvDsInferDims& a, const NvDsInferDims& b) {
  if (a.numDims != b.numDims) return false;
...
bool isValidOutputFormat(const std::string& fmt) {
  static std::unordered_set<std::string> ioFmt{"chw",  "chw2",  "chw4",
                                               "hwc8", "chw16", "chw32"};
  return ioFmt.find(fmt) != ioFmt.end() ? true : false;
}

bool isValidOutputDataType(const std::string& dataType) {
  static std::unordered_set<std::string> ioDataType{"fp32", "fp16", "int32",
                                                    "int8"};
...
nvinfer1::DataType str2DataType(const std::string& dataType) {
  if (!dataType.compare("fp32"))
    ...
    dsInferError("Invalid datatype string %s. Using default kFLOAT datatype",
                 dataType.c_str());
...
uint32_t str2TensorFormat(const std::string& fmt) {
  if (!fmt.compare("chw"))
    ...
    dsInferError("Invalid tensor format string %s. Using default kLINEAR",
                 fmt.c_str());
...
bool validateIOTensorNames(const BuildParams& params,
  ...
  for (auto fmt : params.inputFormats) {
    ...
    for (int i = 0; !found && (i < network.getNbInputs()); ++i) {
      ...
      if (!fmt.first.compare(input->getName())) found = true;
    }
    if (!found) {
  ...
  for (auto fmt : params.outputFormats) {
    ...
    for (int i = 0; !found && (i < network.getNbOutputs()); ++i) {
      ...
      if (!fmt.first.compare(output->getName())) found = true;
    }
    if (!found) {
...
nvinfer1::DataType str2PrecisionType(const std::string& dataType) {
  if (!dataType.compare("fp32"))
    ...
    dsInferError(
        "Invalid precisionType string %s. Using default kFLOAT(fp32) "
        "precisonType",
        dataType.c_str());
...
nvinfer1::DeviceType str2DeviceType(const std::string& deviceType) {
  if (!deviceType.compare("gpu"))
    ...
    dsInferError("Invalid deviceType string %s. Using default kGPU deviceType",
                 deviceType.c_str());
...
__attribute__((visibility("default"))) const char* NvDsInferStatus2Str(
