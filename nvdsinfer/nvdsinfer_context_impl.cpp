...
/* This file contains the implementation of the INvDsInferContext
 * implementation. The pre- and post- processing implementations are also in
 * this file.
 */

/* Pair data type for returning input back to caller. */
using NvDsInferReturnInputPair =
    std::pair<NvDsInferContextReturnInputAsyncFunc, void*>;
...
static void returnInputCudaCallback(cudaStream_t stream, cudaError_t status,
                                    void* userData) {
...
InferPreprocessor::InferPreprocessor(
    const NvDsInferNetworkInfo& info, NvDsInferFormat format,
    const NvDsInferBatchDimsLayerInfo& layerInfo, int id)
    : m_UniqueID(id),
      m_NetworkInfo(info),
      m_NetworkInputFormat(format),
      m_NetworkInputLayer(layerInfo) {}

bool InferPreprocessor::setScaleOffsets(float scale,
  ...
  m_Scale = scale;
  if (!offsets.empty()) {
    m_ChannelMeans.assign(offsets.begin(),
                          offsets.begin() + m_NetworkInfo.channels);
  }
  return true;
}
 ...
 */
NvDsInferStatus InferPreprocessor::readMeanImageFile() {
...
NvDsInferStatus InferPreprocessor::allocateResource() {
  if (!m_MeanFile.empty() || m_ChannelMeans.size() > 0) {
    ...
    m_MeanDataBuffer = std::make_unique<CudaDeviceBuffer>(
        (size_t)m_NetworkInfo.width * m_NetworkInfo.height *
  ...
  /* Create the mean data buffer from per-channel offsets. */
    ...
    std::vector<float> meanData(m_NetworkInfo.channels * m_NetworkInfo.width *
  ...
  nvtx_name = "nvdsinfer_preprocess_complete_uid=" + std::to_string(m_UniqueID);
  nvtxNameCudaEventA(*m_PreProcessCompleteEvent, nvtx_name.c_str());

  return NVDSINFER_SUCCESS;
}

NvDsInferStatus InferPreprocessor::syncStream() {
...
NvDsInferStatus InferPreprocessor::transform(
  ...
  if (waitingEvent) {
    RETURN_CUDA_ERR(cudaStreamWaitEvent(*m_PreProcessStream, *waitingEvent, 0),
  ...
  for (unsigned int i = 0; i < batchSize; i++) {
    ...
    if (convertFcn) {
      ...
      convertFcn(
          outPtr, (unsigned char*)batchInput.inputFrames[i],
      ...
      /* Input needs to be pre-processed. */
      convertFcnFloat(
          outPtr, (float*)batchInput.inputFrames[i], m_NetworkInfo.width,
          m_NetworkInfo.height, batchInput.inputPitch, m_Scale,
          m_MeanDataBuffer.get() ? m_MeanDataBuffer->ptr<float>() : nullptr,
          *m_PreProcessStream);
    }
  }
  ...
  if (batchInput.returnInputFunc) {
    RETURN_CUDA_ERR(
        cudaStreamAddCallback(
            *m_PreProcessStream, returnInputCudaCallback,
            new NvDsInferReturnInputPair(batchInput.returnInputFunc,
                                         batchInput.returnFuncData),
            0),
 ...
 */
NvDsInferStatus InferPostprocessor::parseLabelsFile(
...
NvDsInferStatus InferPostprocessor::allocDeviceResource() {
  return NVDSINFER_SUCCESS;
}

NvDsInferStatus InferPostprocessor::initResource(
  ...
  if (!string_empty(initParams.labelsFilePath)) {
    RETURN_NVINFER_ERROR(parseLabelsFile(initParams.labelsFilePath),
                         "parse label file:%s failed",
                         initParams.labelsFilePath);
  }

  RETURN_NVINFER_ERROR(allocDeviceResource(),
                       "allocate device resource failed");
  return NVDSINFER_SUCCESS;
}

NvDsInferStatus InferPostprocessor::copyBuffersToHostMemory(
    NvDsInferBatch& batch, CudaStream& mainStream) {
...
NvDsInferStatus InferPostprocessor::postProcessHost(
    NvDsInferBatch& batch, NvDsInferContextBatchOutput& batchOutput) {
    ...
    for (unsigned int i = 0; i < m_OutputLayerInfo.size(); i++) {
      ...
      info.buffer =
          (void*)(batch.m_HostBuffers[info.bindingIndex]->ptr<uint8_t>() +
                  info.inferDims.numElements * getElementSize(info.dataType) *
                      index);
    }
...
void InferPostprocessor::freeBatchOutput(
...
NvDsInferStatus DetectPostprocessor::initResource(
  ...
  if (m_UseDBScan)
    printError(
        " 'useDBScan' parameter has been deprecated. Use 'clusterMode' "
        "instead.");
  ...
  m_PerClassDetectionParams.assign(
      initParams.perClassDetectionParams,
  ...
  m_DetectionParams.perClassPreclusterThreshold.resize(
      initParams.numDetectedClasses);
  m_DetectionParams.perClassPostclusterThreshold.resize(
      initParams.numDetectedClasses);
   ...
   * custom library. */
  if (m_CustomLibHandle && !string_empty(initParams.customBBoxParseFuncName)) {
  ...
  if (m_ClusterMode == NVDSINFER_CLUSTER_DBSCAN ||
      m_ClusterMode == NVDSINFER_CLUSTER_DBSCAN_NMS_HYBRID) {
    m_DBScanHandle.reset(NvDsInferDBScanCreate(),
                         [](NvDsInferDBScanHandle handle) {
                           if (handle) NvDsInferDBScanDestroy(handle);
                         });
    if (!m_DBScanHandle) {
...
NvDsInferStatus DetectPostprocessor::parseEachBatch(
...
NvDsInferStatus InstanceSegmentPostprocessor::initResource(
    const NvDsInferContextInitParams& initParams) {
  RETURN_NVINFER_ERROR(InferPostprocessor::initResource(initParams),
                       "init post processing resource failed");
  ...
  if (m_ClusterMode != NVDSINFER_CLUSTER_NONE) {
    printError(" cluster mode %d not supported with instance segmentation",
               m_ClusterMode);
    return NVDSINFER_CONFIG_FAILED;
  }
  ...
  if (m_CustomLibHandle &&
    ...
    printError(
        "Custom parse function not found for InstanceSegment-postprocessor");
    return NVDSINFER_RESOURCE_ERROR;
  }
...
NvDsInferStatus InstanceSegmentPostprocessor::parseEachBatch(
    const std::vector<NvDsInferLayerInfo>& outputLayers,
    NvDsInferFrameOutput& result) {
...
NvDsInferStatus ClassifyPostprocessor::initResource(
    const NvDsInferContextInitParams& initParams) {
  RETURN_NVINFER_ERROR(InferPostprocessor::initResource(initParams),
                       "init post processing resource failed");
...
NvDsInferStatus ClassifyPostprocessor::parseEachBatch(
    const std::vector<NvDsInferLayerInfo>& outputLayers,
    NvDsInferFrameOutput& result) {
...
NvDsInferStatus SegmentPostprocessor::initResource(
    const NvDsInferContextInitParams& initParams) {
  RETURN_NVINFER_ERROR(InferPostprocessor::initResource(initParams),
                       "init post processing resource failed");
...
NvDsInferStatus OtherPostprocessor::initResource(
    const NvDsInferContextInitParams& initParams) {
  return NVDSINFER_SUCCESS;
}
...
NvDsInferStatus NvDsInferContextImpl::preparePreprocess(
    const NvDsInferContextInitParams& initParams) {
  assert(!m_Preprocessor);
  assert(m_NetworkInfo.channels && m_NetworkInfo.width && m_NetworkInfo.height);
  ...
  std::unique_ptr<InferPreprocessor> processor =
      std::make_unique<InferPreprocessor>(m_NetworkInfo,
                                          ...
                                          m_InputImageLayerInfo, m_UniqueID);
  assert(processor);

  processor->setLoggingFunc(m_LoggingFunc);

  if (initParams.networkScaleFactor > 0.0f) {
    std::vector<float> offsets(initParams.offsets,
                               initParams.offsets + initParams.numOffsets);
    if (!processor->setScaleOffsets(initParams.networkScaleFactor, offsets)) {
...
NvDsInferStatus NvDsInferContextImpl::preparePostprocess(
    const NvDsInferContextInitParams& initParams) {
  ...
  switch (initParams.networkType) {
    ...
    case NvDsInferNetworkType_Classifier:
      processor = std::make_unique<ClassifyPostprocessor>(m_UniqueID, m_GpuID);
      break;
    ...
    case NvDsInferNetworkType_InstanceSegmentation:
      processor =
          std::make_unique<InstanceSegmentPostprocessor>(m_UniqueID, m_GpuID);
      break;
  ...
  RETURN_NVINFER_ERROR(
      processor->initResource(initParams),
...
NvDsInferStatus NvDsInferContextImpl::initInferenceInfo(
  ...
  for (int i = 0; i < layerSize; i++) {
    ...
    if (!info.isInput) m_OutputLayerInfo.push_back(info);
  }
 ...
 * engine. */
NvDsInferStatus NvDsInferContextImpl::initialize(
  ...
  m_LoggingFunc = [this, userCtx, logFunc, uniqueID](NvDsInferLogLevel level,
                                                     const char* msg) {
  ...
  if (initParams.numLayerDevicePrecisions > 0 &&
    ...
    printError(
        "numLayerDevicePrecisions >0 but layerDevicePrecisions array not "
        "specified");
    return NVDSINFER_CONFIG_FAILED;
  }
  ...
  RETURN_CUDA_ERR(cudaSetDevice(m_GpuID), "Failed to set cuda device (%d).",
                  m_GpuID);
 ...
 *
 * NvDsInfer enqueue API requires an array of (void *) buffer pointers. The
 * length of the array is equal to the number of bound layers. The buffer
 * corresponding to a layer is placed at an index equal to the layer's binding
 * index.
 *
 ...
 */
NvDsInferStatus NvDsInferContextImpl::allocateBuffers() {
  ...
  nvtx_name = "nvdsinfer_TRT_input_consumed_uid=" + std::to_string(m_UniqueID);
  ...
  for (size_t iB = 0; iB < m_Batches.size(); iB++) {
    ...
    for (unsigned int jL = 0; jL < m_AllLayerInfo.size(); jL++) {
      ...
      size_t size = m_MaxBatchSize * bindingDims.numElements *
                    getElementSize(layerInfo.dataType);
      ...
      if (layerInfo.isInput && !m_Postprocessor->needInputCopy()) continue;
...
NvDsInferStatus NvDsInferContextImpl::queueInputBatch(
  ...
  RETURN_NVINFER_ERROR(
      m_Preprocessor->transform(batchInput, m_BindingBuffers[INPUT_LAYER_INDEX],
  ...
  auto recyleFunc = [this](NvDsInferBatch* batch) {
    if (batch) m_FreeBatchQueue.push(batch);
  };
  ...
  auto backendBuffer =
      std::make_shared<BackendBatchBuffer>(bindings, m_AllLayerInfo, batchSize);
  assert(m_BackendContext && backendBuffer);
  assert(m_InferStream && m_InputConsumedEvent && m_InferCompleteEvent);

  RETURN_NVINFER_ERROR(
      m_BackendContext->enqueueBuffer(backendBuffer, *m_InferStream,
                                      m_InputConsumedEvent.get()),
...
NvDsInferStatus NvDsInferContextImpl::dequeueOutputBatch(
    NvDsInferContextBatchOutput& batchOutput) {
  assert(m_Initialized);
  auto recyleFunc = [this](NvDsInferBatch* batch) {
    if (batch) m_FreeBatchQueue.push(batch);
 ...
 */
void NvDsInferContextImpl::releaseBatchOutput(
    NvDsInferContextBatchOutput& batchOutput) {
 ...
 */
void NvDsInferContextImpl::fillLayersInfo(
...
const std::vector<std::vector<std::string>>& NvDsInferContextImpl::getLabels() {
  assert(m_Postprocessor);
  return m_Postprocessor->getLabels();
}

/* Check if the runtime backend is compatible with requested configuration. */
NvDsInferStatus NvDsInferContextImpl::checkBackendParams(
  ...
  if (initParams.inferInputDims.c && initParams.inferInputDims.h &&
      initParams.inferInputDims.w) {
    NvDsInferDims inputDims = {
        3,
        {initParams.inferInputDims.c, initParams.inferInputDims.h,
         initParams.inferInputDims.w},
        0};
    NvDsInferBatchDims requestBatchDims{(int)initParams.maxBatchSize,
                                        inputDims};
...
bool NvDsInferContextImpl::deserializeEngineAndBackend(
    const std::string enginePath, int dla, std::shared_ptr<TrtEngine>& engine,
    std::unique_ptr<BackendContext>& backend) {
  auto builder = std::make_unique<TrtModelBuilder>(m_GpuID, *gTrtLogger,
                                                   m_CustomLibHandle);
  assert(builder);
  ...
  if (!newEngine) {
    printWarning("deserialize engine from file :%s failed",
                 safeStr(enginePath));
    return false;
  }
 ...
 * backend */
std::unique_ptr<BackendContext> NvDsInferContextImpl::buildModel(
    NvDsInferContextInitParams& initParams) {
  printInfo("Trying to create engine from model files");

  std::unique_ptr<TrtModelBuilder> builder = std::make_unique<TrtModelBuilder>(
  ...
  if (builder->serializeEngine(enginePath, engine->engine()) !=
      NVDSINFER_SUCCESS) {
    printWarning("failed to serialize cude engine to file: %s",
                 safeStr(enginePath));
  } else {
  ...
  if (checkBackendParams(*backend, initParams) != NVDSINFER_SUCCESS) {
    printError("deserialized backend context :%s failed to match config params",
               safeStr(enginePath));
    return nullptr;
  }
...
std::unique_ptr<BackendContext> NvDsInferContextImpl::generateBackendContext(
    NvDsInferContextInitParams& initParams) {
  int dla = -1;
  if (initParams.useDLA && initParams.dlaCore >= 0) dla = initParams.dlaCore;

  std::shared_ptr<TrtEngine> engine;
  std::unique_ptr<BackendContext> backend;
  if (!string_empty(initParams.modelEngineFilePath)) {
    if (!deserializeEngineAndBackend(initParams.modelEngineFilePath, dla,
                                     engine, backend)) {
      printWarning(
...
NvDsInferContextImpl::~NvDsInferContextImpl() {
  ...
  if (m_Preprocessor) m_Preprocessor->syncStream();
 ...
 */
void NvDsInferContextImpl::destroy() { delete this; }
...
/*
 * Factory function to create an NvDsInferContext instance and initialize it
