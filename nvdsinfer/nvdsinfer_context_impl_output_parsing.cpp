...
bool DetectPostprocessor::parseBoundingBox(
  ...
  getDimsCHWFromDims(outputBBoxDims,
                     outputLayersInfo[outputBBoxLayerIndex].inferDims);
  ...
  for (unsigned int classIndex = 0; classIndex < numClasses; classIndex++) {
    ...
    for (unsigned int h = 0; h < outputCoverageDims.h; h++) {
      for (unsigned int w = 0; w < outputCoverageDims.w; w++) {
        ...
        if (confidence <
            detectionParams.perClassPreclusterThreshold[classIndex])
          continue;
        ...
        if (rectX1Float < 0) rectX1Float = 0;
        if (rectX2Float < 0) rectX2Float = 0;
        if (rectY1Float < 0) rectY1Float = 0;
        if (rectY2Float < 0) rectY2Float = 0;
          ...
          [detectionParams](const NvDsInferObjectDetectionInfo& obj) {
            return (obj.classId >= detectionParams.numClassesConfigured) ||
                           (obj.detectionConfidence <
                            detectionParams
                                .perClassPreclusterThreshold[obj.classId])
                       ? true
                       : false;
          }),
      objectList.end());
 ...
 * rest
 */
void DetectPostprocessor::filterTopKOutputs(
    const int topK, std::vector<NvDsInferObjectDetectionInfo>& objectList) {
  if (topK < 0) return;

  std::stable_sort(objectList.begin(), objectList.end(),
                   [](const NvDsInferObjectDetectionInfo& obj1,
                      const NvDsInferObjectDetectionInfo& obj2) {
                     return obj1.detectionConfidence > obj2.detectionConfidence;
                   });
  objectList.resize(static_cast<size_t>(topK) <= objectList.size()
                        ? topK
                        : objectList.size());
}
...
std::vector<int> DetectPostprocessor::nonMaximumSuppression(
    ...
    std::vector<NvDsInferParseObjectInfo>& bbox, const float nmsThreshold) {
  ...
  auto computeIoU = [&overlap1D](NvDsInferParseObjectInfo& bbox1,
                                 NvDsInferParseObjectInfo& bbox2) -> float {
...
void DetectPostprocessor::clusterAndFillDetectionOutputNMS(
  ...
  auto maxComp =
      [](const std::vector<NvDsInferObjectDetectionInfo>& c1,
  ...
  for (unsigned int c = 0; c < m_NumDetectedClasses; c++) {
    if (!m_PerClassObjectList[c].empty()) {
      ...
      const std::vector<int> indices =
          nonMaximumSuppression(scoreIndex, m_PerClassObjectList[c],
      ...
      clusteredBboxes.insert(clusteredBboxes.end(), postNMSBboxes.begin(),
                             postNMSBboxes.end());
    }
  }
...
void DetectPostprocessor::clusterAndFillDetectionOutputDBSCAN(
  ...
  for (unsigned int c = 0; c < m_NumDetectedClasses; c++) {
    ...
    m_PerClassObjectList[c].erase(
        std::remove_if(
            m_PerClassObjectList[c].begin(), m_PerClassObjectList[c].end(),
            [detectionParams](const NvDsInferObjectDetectionInfo& obj) {
 ...
 */
void DetectPostprocessor::clusterAndFillDetectionOutputHybrid(
    NvDsInferDetectionOutput& output) {
  NvDsInferDBScanClusteringParams clusteringParams;
  clusteringParams.enableATHRFilter = ATHR_ENABLED;
  clusteringParams.thresholdATHR = ATHR_THRESHOLD;
  ...
  for (unsigned int c = 0; c < m_NumDetectedClasses; c++) {
    ...
    if (m_PerClassDetectionParams[c].minBoxes > 0) {
      NvDsInferDBScanClusterHybrid(m_DBScanHandle.get(), &clusteringParams,
                                   objArray, &numObjects);
    }
    m_PerClassObjectList[c].resize(numObjects);
  }
...
void InstanceSegmentPostprocessor::fillUnclusteredOutput(
    NvDsInferDetectionOutput& output) {
  ...
  for (unsigned int c = 0; c < m_NumDetectedClasses; c++) {
    filterTopKOutputs(m_PerClassDetectionParams.at(c).topK,
                      m_PerClassInstanceMaskList.at(c));
 ...
 * rest
 */
void InstanceSegmentPostprocessor::filterTopKOutputs(
    const int topK, std::vector<NvDsInferInstanceMaskInfo>& objectList) {
  if (topK < 0) return;

  std::stable_sort(objectList.begin(), objectList.end(),
                   [](const NvDsInferInstanceMaskInfo& obj1,
                      const NvDsInferInstanceMaskInfo& obj2) {
                     return obj1.detectionConfidence > obj2.detectionConfidence;
                   });
  objectList.resize(static_cast<size_t>(topK) <= objectList.size()
                        ? topK
...
NvDsInferStatus InstanceSegmentPostprocessor::fillDetectionOutput(
  ...
  for (auto& list : m_PerClassInstanceMaskList) list.clear();
  ...
  if (m_CustomParseFunc) {
    if (!m_CustomParseFunc(outputLayers, m_NetworkInfo, m_DetectionParams,
                           m_InstanceMaskList)) {
      printError(
          "Failed to parse bboxes and instance mask using custom parse "
          "function");
...
NvDsInferStatus DetectPostprocessor::fillDetectionOutput(
  ...
  /* Clear all per class object lists */
  for (auto& list : m_PerClassObjectList) list.clear();

  /* Call custom parsing function if specified otherwise use the one
   * written along with this implementation. */
  if (m_CustomBBoxParseFunc) {
    if (!m_CustomBBoxParseFunc(outputLayers, m_NetworkInfo, m_DetectionParams,
                               m_ObjectList)) {
      printError("Failed to parse bboxes using custom parse function");
      return NVDSINFER_CUSTOM_LIB_FAILED;
    }
  } else {
    if (!parseBoundingBox(outputLayers, m_NetworkInfo, m_DetectionParams,
                          m_ObjectList)) {
...
NvDsInferStatus ClassifyPostprocessor::fillClassificationOutput(
  ...
  if (m_CustomClassifierParseFunc) {
    if (!m_CustomClassifierParseFunc(outputLayers, m_NetworkInfo,
                                     m_ClassifierThreshold, attributes,
                                     attrString)) {
      printError(
  ...
  for (size_t i = 0; i < output.numAttributes; i++) {
    ...
    output.attributes[i].attributeConfidence =
        attributes[i].attributeConfidence;
...
void InferPostprocessor::releaseFrameOutput(NvDsInferFrameOutput& frameOutput) {
  switch (frameOutput.outputType) {
    ...
    case NvDsInferNetworkType_Classifier:
      for (unsigned int j = 0;
           j < frameOutput.classificationOutput.numAttributes; j++) {
