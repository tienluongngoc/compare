...
class InferPreprocessor {
 public:
  ...
  NvDsInferStatus transform(NvDsInferContextBatchInput& batchInput,
                            void* devBuf, CudaStream& mainStream,
...
class InferPostprocessor {
 ...
 public:
  ...
  virtual NvDsInferStatus copyBuffersToHostMemory(NvDsInferBatch& buffer,
                                                  CudaStream& mainStream);

  virtual NvDsInferStatus postProcessHost(NvDsInferBatch& buffer,
...
class InstanceSegmentPostprocessor : public InferPostprocessor {
 public:
  InstanceSegmentPostprocessor(int id, int gpuId = 0)
      : InferPostprocessor(NvDsInferNetworkType_InstanceSegmentation, id,
                           gpuId) {}
 ...
 private:
  ...
  std::vector<std::vector<NvDsInferInstanceMaskInfo>>
...
class NvDsInferContextImpl : public INvDsInferContext {
 ...
 private:
  ...
  NvDsInferStatus queueInputBatch(
      NvDsInferContextBatchInput& batchInput) override;
  NvDsInferStatus dequeueOutputBatch(
      NvDsInferContextBatchOutput& batchOutput) override;
