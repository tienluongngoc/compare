...
constexpr nvinfer1::DataType kDefaultTensorDataType =
...
NvDsInferStatus CaffeModelParser::setPluginFactory() {
  assert(m_CaffeParser);
  if (!m_LibHandle) return NVDSINFER_SUCCESS;
  ...
  if (!fcn) return NVDSINFER_SUCCESS;
...
NvDsInferStatus CaffeModelParser::parseModel(
  ...
  if (!blobNameToTensor) {
    dsInferError("Failed while parsing caffe network: %s",
                 safeStr(m_ProtoPath));
    return NVDSINFER_TENSORRT_ERROR;
  }
...
UffModelParser::UffModelParser(const NvDsInferContextInitParams& initParams,
  ...
  if (m_ModelParams.inputDims.size() != m_ModelParams.inputNames.size()) {
    dsInferError("Unrecognized uff input blob names and dims are not match");
    return;
  }
...
NvDsInferStatus UffModelParser::initParser() {
  ...
  for (size_t i = 0; i < m_ModelParams.inputNames.size(); ++i) {
    if (!m_UffParser->registerInput(m_ModelParams.inputNames[i].c_str(),
                                    m_ModelParams.inputDims[i],
                                    m_ModelParams.inputOrder)) {
      dsInferError(
  ...
  for (const auto& layerName : m_ModelParams.outputNames) {
    if (!m_UffParser->registerOutput(layerName.c_str())) {
      dsInferError("Failed to register uff output blob: %s",
                   safeStr(layerName));
      return NVDSINFER_CONFIG_FAILED;
    }
  }
...
NvDsInferStatus UffModelParser::parseModel(
    nvinfer1::INetworkDefinition& network) {
  if (!isValid()) {
  ...
  if (!file_accessible(m_ModelParams.uffFilePath)) {
    dsInferError("Cannot access UFF file '%s'",
                 safeStr(m_ModelParams.uffFilePath));
    return NVDSINFER_CONFIG_FAILED;
  }
...
NvDsInferStatus OnnxModelParser::parseModel(
    nvinfer1::INetworkDefinition& network) {
...
CustomModelParser::CustomModelParser(
  ...
  if (!modelParser) {
    dsInferError("Failed to create custom parser from lib:%s, model path:%s",
...
NvDsInferStatus CustomModelParser::parseModel(
    nvinfer1::INetworkDefinition& network) {
  if (!isValid()) {
    dsInferError(
...
NvDsInferStatus ImplicitBuildParams::configBuilder(
    TrtModelBuilder& trtBuilder) {
  return trtBuilder.configImplicitOptions(*this);
}

bool ExplicitBuildParams::sanityCheck() const {
  /* Check that min <= opt <= max batch size. */
  if (minBatchSize > optBatchSize || optBatchSize > maxBatchSize) return false;

  for (auto& layer : inputProfileDims) {
    int nd = -1;
    if (!std::all_of(layer.begin(), layer.end(),
                     [&nd](const nvinfer1::Dims& s) {
                       if (nd > 0) return nd == s.nbDims;
                       nd = s.nbDims;
                       return true;
                     })) {
...
NvDsInferStatus ExplicitBuildParams::configBuilder(
    TrtModelBuilder& trtBuilder) {
...
TrtEngine::TrtEngine(UniquePtrWDestroy<nvinfer1::ICudaEngine>&& engine,
                     ...
                     int dlaCore, const std::shared_ptr<DlLibHandle>& dlHandle,
 ...
 */
NvDsInferStatus TrtEngine::getLayerInfo(int idx, NvDsInferLayerInfo& info) {
  assert(m_Engine);
  ...
  switch (m_Engine->getBindingDataType(idx)) {
    ...
    case nvinfer1::DataType::kINT8:
      ...
      break;
    default:
      dsInferError("Unknown data type for bound layer i(%s)",
                   safeStr(info.layerName));
      return NVDSINFER_TENSORRT_ERROR;
  }
  return NVDSINFER_SUCCESS;
}
...
NvDsInferStatus TrtEngine::getImplicitLayersInfo(
...
NvDsInferStatus TrtEngine::getFullDimsLayersInfo(
...
std::unique_ptr<TrtEngine> TrtModelBuilder::getCudaEngineFromCustomLib(
    ...
    if (m_Builder->platformHasFastInt8()) {
      if (m_Int8Calibrator != nullptr) {
        ...
        modelDataType = nvinfer1::DataType::kFLOAT;
                 ...
                 cudaEngineGetDeprecatedFcn != nullptr) {
        dsInferWarning(
  ...
  if (cudaEngineGetFcn &&
      (!cudaEngineGetFcn(m_Builder.get(),
  ...
  if (cudaEngineGetDeprecatedFcn &&
      (!cudaEngineGetDeprecatedFcn(m_Builder.get(),
                                   (NvDsInferContextInitParams*)&initParams,
                                   modelDataType, engine) ||
       engine == nullptr)) {
    dsInferError(
  ...
  return std::make_unique<TrtEngine>(
      UniquePtrWDestroy<nvinfer1::ICudaEngine>(engine), dla);
}

/* Build the model and return the generated engine. */
std::unique_ptr<TrtEngine> TrtModelBuilder::buildModel(
    const NvDsInferContextInitParams& initParams,
  ...
  if (m_DlLib && !string_empty(initParams.customEngineCreateFuncName)) {
    ...
    if (!cudaEngineGetFcn) {
      dsInferError(
          "Could not find Custom Engine Creation Function '%s' in custom lib",
          initParams.customEngineCreateFuncName);
      return nullptr;
    }
  }
  if (m_DlLib && cudaEngineGetFcn == nullptr)
    cudaEngineGetDeprecatedFcn =
        m_DlLib->symbol<NvDsInferCudaEngineGetFcnDeprecated>(
  ...
  if (cudaEngineGetFcn || cudaEngineGetDeprecatedFcn ||
    ...
    engine = getCudaEngineFromCustomLib(
        cudaEngineGetDeprecatedFcn, cudaEngineGetFcn, initParams, networkMode);
    if (engine == nullptr) {
  ...
  suggestedPathName = modelPath + "_b" +
                      std::to_string(initParams.maxBatchSize) + "_" + devId +
                      "_" + networkMode2Str(networkMode) + ".engine";
  return engine;
}

NvDsInferStatus TrtModelBuilder::buildNetwork(
    const NvDsInferContextInitParams& initParams) {
  ...
  for (unsigned int i = 0; i < initParams.numOutputIOFormats; ++i) {
    ...
    if (!isValidOutputFormat(format)) {
      dsInferError("Invalid output data format specified %s", format.c_str());
      return NVDSINFER_CONFIG_FAILED;
    }
  }
  for (unsigned int i = 0; i < initParams.numLayerDevicePrecisions; ++i) {
    ...
    std::string precisionType =
        outputDevicePrecision.substr(pos1 + 1, pos2 - pos1 - 1);
    ...
    if (!isValidDeviceType(deviceType)) {
      dsInferError("Invalid deviceType specified %s", deviceType.c_str());
      return NVDSINFER_CONFIG_FAILED;
    }
  }
...
std::unique_ptr<BuildParams> TrtModelBuilder::createImplicitParams(
    const NvDsInferContextInitParams& initParams) {
...
std::unique_ptr<BuildParams> TrtModelBuilder::createDynamicParams(
    const NvDsInferContextInitParams& initParams) {
...
void TrtModelBuilder::initCommonParams(
  ...
  for (unsigned int i = 0; i < initParams.numOutputIOFormats; ++i) {
    ...
    std::pair<std::string, BuildParams::TensorIOFormat> outputFmt{layerName,
                                                                  fmt};
  ...
  for (unsigned int i = 0; i < initParams.numLayerDevicePrecisions; ++i) {
    ...
    std::string precisionType =
        outputDevicePrecision.substr(pos1 + 1, pos2 - pos1 - 1);
    std::string deviceType = outputDevicePrecision.substr(pos2 + 1);
    BuildParams::LayerDevicePrecision fmt = std::make_tuple(
        str2PrecisionType(precisionType), str2DeviceType(deviceType));
    std::pair<std::string, BuildParams::LayerDevicePrecision> outputFmt{
        layerName, fmt};
    params.layerDevicePrecisions.insert(outputFmt);
  }
}

std::unique_ptr<TrtEngine> TrtModelBuilder::buildEngine() {
  assert(m_Builder);
  assert(m_Network);
  assert(m_Options);
  assert(m_Parser);
...
std::unique_ptr<TrtEngine> TrtModelBuilder::buildEngine(
  ...
  /* Configure m_BuilderConfig with one of ImplicitBuildParams
   * (configImplicitOptions()) or ExplicitBuildParams
   * (configExplicitOptions()).*/
...
NvDsInferStatus TrtModelBuilder::configCommonOptions(BuildParams& params) {
  ...
  if (params.dlaCore >= 0) {
    ...
    if (params.networkMode != NvDsInferNetworkMode_INT8) {
      ...
      dsInferWarning(
          "DLA does not support FP32 precision type, using FP16 mode.");
      builderConfig.setFlag(nvinfer1::BuilderFlag::kFP16);
    }
  }
...
NvDsInferStatus TrtModelBuilder::configImplicitOptions(
    ImplicitBuildParams& params) {
...
NvDsInferStatus TrtModelBuilder::configExplicitOptions(
    ExplicitBuildParams& params) {
  assert(m_Builder && m_Network && m_BuilderConfig);
  nvinfer1::IBuilder& builder = *m_Builder;
  nvinfer1::INetworkDefinition& network = *m_Network;
  nvinfer1::IBuilderConfig& builderConfig = *m_BuilderConfig;
  ...
  for (; iL < (int)params.inputProfileDims.size(); iL++) {
    ...
    nvinfer1::Dims minDims =
        params.inputProfileDims.at(iL)[(int)nvinfer1::OptProfileSelector::kMIN];

    if (minDims.nbDims + 1 != modelDims.nbDims) {
      dsInferError("explict dims.nbDims in config does not match model dims.");
      return NVDSINFER_CONFIG_FAILED;
    }
  ...
  for (; iL < network.getNbInputs(); ++iL) {
    ...
    if (std::any_of(dims.d, dims.d + dims.nbDims,
                    [](int d) { return d < 0; })) {
...
NvDsInferStatus TrtModelBuilder::serializeEngine(
...
std::unique_ptr<TrtEngine> TrtModelBuilder::deserializeEngine(
    const std::string& path, int dla) {
  std::ifstream fileIn(path, std::ios::binary);
  if (!fileIn.is_open()) {
    dsInferError("Deserialize engine failed because file path: %s open error",
                 safeStr(path));
    return nullptr;
  }
  ...
  return std::make_unique<TrtEngine>(std::move(engine), runtime, dla, m_DlLib,
                                     factory);
}
